function henry(runtime, observer) {
    const main = runtime.module();
    const fileAttachments = new Map([["nations.json", new URL("../data/facebookData2.json", 'http://localhost:4200/assets/files/facebookData.json')]]);
    ...
    main.variable(observer("viewof year")).define("viewof year", ["Scrubber", "d3"], function (Scrubber, d3) {
      return (
  ...
  span.observablehq--inspect {
    ...
    main.variable(observer()).define(["chart", "currentData"], function (chart, currentData) {
      return (
        chart.update(currentData)
      )
    });
    main.variable(observer("currentData")).define("currentData", ["dataAt", "year"], function (dataAt, year) {
      return (
        dataAt(year)
      )
    });
    ...
    main.variable(observer("y")).define("y", ["d3", "height", "margin"], function (d3, height, margin) {
      return (
        d3.scaleLinear([14, 86], [height - margin.bottom, margin.top])
      )
    });
    main.variable(observer("radius")).define("radius", ["d3", "width"], function (d3, width) {
      return (
    ...
    main.variable(observer("grid")).define("grid", ["x", "margin", "height", "y", "width"], function (x, margin, height, y, width) {
      return (
        g => g
    ...
    main.variable(observer("dataAt")).define("dataAt", ["data", "valueAt"], function (data, valueAt) {
      return (
    ...
    main.variable(observer("valueAt")).define("valueAt", ["bisectYear"], function (bisectYear) {
      return (
    ...
    main.variable(observer("bisectYear")).define("bisectYear", ["d3"], function (d3) {
      return (
  ...
  ## Implementation`
    ...
    main.variable(observer("Scrubber")).define("Scrubber", ["html", "Generators"], function (html, Generators) {
      return (
