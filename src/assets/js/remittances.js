$(function() {
  ...
  var projection = d3.geo
    .projection(d3.geo.hammer.raw(1.75, 2))
    .rotate([-10, -45])
    .scale(180);
  ...
  var timelineSvg = d3
    ...
    .attr(
      "width",
      timelineWidth + timelineMargins.left + timelineMargins.right
    );
  ...
  var timeline = timelineSvg
    ...
    .attr(
      "transform",
      "translate(" + timelineMargins.left + "," + timelineMargins.top + ")"
    );

  $("#timeline svg").attr(
    "height",
    timelineHeight + timelineMargins.top + timelineMargins.bottom
  );
  ...
  var remittanceTotals,
    remittanceTotalsByMigrantsOrigin,
    remittanceTotalsPerMigrant,
    remittanceTotalsPerMigrantByMigrantsOrigin,
    maxRemittanceValue,
    maxRemittancePerMigrantValue,
    migrationTotals,
    migrationTotalsByOrigin,
    aidTotals,
    aidTotalsByRecipient;

  var yearScale = d3.scale.linear().domain(remittanceYearsDomain);

  var tseriesScale = d3.scale.linear().range([timelineHeight, 2]);

  var tseriesLine = d3.svg
    .line()
  ...
  function slideSelected() {
    ...
    switch (mySwiper.activeSlide) {
      ...
      case 4: // Weniger Geld fÃ¼r Griechenland und die TÃ¼rkei
        $("#color-legend").fadeIn();
        setPerMigrant(false);
  ...
  function initTimeSeries(name) {
    ...
    if (tseries.empty()) {
      tseries = timeline.append("g").attr("class", "tseries");
    }
  ...
  function updateTimeSeries() {
    ...
    if (perMigrant) {
      ...
      d3.select("#timeline g.tseries .legend .remittances text").text(
        msg("details.tseries.legend.remittances.per-capita")
      );
    } else {
      if (country == null) {
      ...
      d3.select("#timeline g.tseries .legend .remittances text").text(
        msg("details.tseries.legend.remittances")
      );
    }
    ...
    if (perMigrant) {
      d3.selectAll("#timeline g.tseries .aid").attr("visibility", "hidden");
    } else {
      d3.selectAll("#timeline g.tseries .aid").attr("visibility", "visible");
  ...
  function updateDetails() {
    ...
    details.select(".year").text(msg("details.remittances.year", selectedYear));
    ...
    if (highlightedCountry != null || selectedCountry != null) {
      ...
      totalRemittances =
        countryRem != null ? str2num(countryRem[selectedYear]) : NaN;
      ...
      details
        .select(".aid .title")
        .text(msg("details.aid.title.selected-country"));
      details
        .select(".migrants .title")
        .text(msg("details.migrants.title.selected-country"));
      details
        .select(".remittances .title")
        .text(msg("details.remittances.title.selected-country"));
    } else {
      ...
      details
        .select(".migrants .title")
        .text(msg("details.migrants.title.total"));
      details
        .select(".remittances .title")
        .text(msg("details.remittances.title.total"));
    }

    details.select(".migrants .value").text(numberFormat(numMigrants));
    details
      .select(".remittances .value")
      .text(moneyMillionsFormat(totalRemittances));
    details.select(".aid .value").text(moneyMillionsFormat(totalAid));
    details
      .select(".remittancesPerCapita .value")
      .text(moneyMillionsFormat(totalRemittances / numMigrants));
  ...
  function selectYear(year, duration) {
    ...
    t.select("#timeline g.selectorHand").attr(
      "transform",
      "translate(" + yearScale(year) + ",0)"
    );
  ...
  $(document).keyup(function(e) {
    if (e.keyCode == 27) selectCountry(null);
  });
  ...
  function highlightCountry(code) {
    ...
    chart_svg.selectAll("path.land").sort(function(a, b) {
  ...
  function updateChoropleth() {
    ...
    if (selectedCountry === null && highlightedCountry == null) {
      ...
      var code =
        selectedCountry !== null ? selectedCountry : highlightedCountry;
      ...
      chart_svg
        ...
        .classed("highlighted", function(d) {
          return d.id === highlightedCountry;
        })
        ...
        .attr("fill", function(d) {
          ...
          if (m !== undefined) {
            ...
            if (!isNaN(val) && val > 0 /* for log scale to work*/)
  ...
  function interpolateNumOfMigrants(values, year) {
    ...
    if (isNaN(val)) {
      if (year >= 2011) {
        ...
        var l = Math.floor(year / 10) * 10,
          r = Math.ceil(year / 10) * 10;
        var t = (year - l) / (r - l);
  ...
  function calcRemittanceTotalsPerMigrantByMigrantsOrigin() {
    var result = {},
      c,
      ci,
      countries = d3.keys(remittanceTotalsByMigrantsOrigin);
  ...
  function updateCircleLegend() {
    ...
    var svg,
      defs,
      g = container.select("g.circle-legend"),
      itemEnter;
    ...
    itemEnter.append("text").attr("x", maxr + 50 + 5);

    // update
    var items = g.selectAll("g.item").attr("transform", function(d) {
      return "translate(0," + (maxr * 2 - 2 * rscale(d)) + ")";
    });
    ...
    items.selectAll("text").text(function(d) {
      return moneyMillionsFormat(d);
    });
  }
  ...
  function updateColorLegend() {
    ...
    var svg,
      defs,
      g = container.select("g.color-legend");
  ...
  queue()
    .defer(d3.json, "./assets/lib/state.json")
    .defer(d3.csv, "./assets/lib/remittances.csv")
    .defer(d3.json, "./assets/lib/oecd-aid.json") // NOTE: there are is TOTAL
    .defer(d3.csv, "./assets/lib/migrations.csv") // filtered by > 100
    .defer(d3.csv, "./assets/lib/migration-totals.csv") // we need to load it separately, not calculate
    ...
    .await(function(err, world, remittances, aid, migrations, migrationTotals) {
      migrationTotals = migrationTotals.filter(function(m) {
        return ["FRO", "MNE", "NCL", "PYF", "BMU", "NGA"].indexOf(m.origin) < 0; // remove outliers, probably wrong data
      });

      $("#loading").hide();
      //    yearAnimation.start();

      remittanceTotalsByMigrantsOrigin = d3 //nestBy("iso3", remittances);
        .nest()
        .key(function(d) {
          return d.iso3;
        })
      ...
      var updateMap = function() {
        chart_svg.selectAll("g.map path").attr("d", path);
      };
      ...
      var gcountries = chart_svg.append("g").attr("class", "countries");

      maxRemittanceValue = d3.max(remittances, function(d) {
        return d3.max(
          remittanceYears.map(function(y) {
            return +d[y];
          })
        );
      });
      ...
      var updateBubblePositions = function() {
        gcountries
          ...
          //        });
          .attr("cx", function(d) {
            if (d.centroid) return projection(d.centroid)[0];
          })
          .attr("cy", function(d) {
            if (d.centroid) return projection(d.centroid)[1];
          });
      };
      ...
      var timelineAxisGroup = timeline
        .append("g")
        .attr("class", "timeAxis")
        .attr("transform", "translate(0," + timelineHeight + ")");

      var timelineRightAxisGroup = timeline
        .append("g")
      ...
      var selectorHandDrag = d3.behavior
        .drag()
        .origin(Object)
      ...
      d3.select("#timeline g.chart").on("click", function() {
        var c = d3.mouse(this);
        ...
        .on("mousemove", function(e) {
          ...
          if (selectedCountry != null) {
            if (selectedCountry !== iso3) {
              val = getInterpolatedNumberOfMigrants(
                selectedCountry,
                iso3,
                selectedYear
              );
              text =
                "<b>" +
                countryNamesByCode[iso3] +
                "</b>" +
                (!isNaN(val)
                  ? ": <br>" +
                    msg(
                      "tooltip.migrants.number.from-a",
                      numberFormat(val),
                      countryNamesByCode[selectedCountry]
                    )
                  : ": " + numberFormat(val));
              ...
              if (vals != null) {
                ...
                text =
                  ...
                  (!isNaN(val)
                    ? ": <br>" +
                      msg(
                        "tooltip.remittances.amount",
                        moneyMillionsFormat(val)
                      )
                    : ": " + numberFormat(val));
              }
            }
